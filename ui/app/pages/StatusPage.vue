                            ...
                            >
                                ...
                                >
                                    <div class="account-info">
                                        ...
                                        <AccountTierEditor
                                            v-if="!item.isInvalid"
                                            :model-value="item.accountTier || 'default'"
                                            :disabled="isBusy"
                                            @update="value => updateAccountTier(item.index, value)"
                                        />
                                        <span v-else class="account-tier-unavailable">
                                            {{ t("accountTierUnavailable") }}
                                        </span>
                                    </div>
                                </el-tooltip>
...
const updateAccountTier = async (targetIndex, accountTier) => {
    const targetAccount = state.accountDetails.find(acc => acc.index === targetIndex);
    if (!targetAccount || targetAccount.isInvalid) {
        ElMessage.error(t("errorAccountTierNotEditable"));
        return;
    }

    if (targetAccount.accountTier === accountTier) {
        return;
    }

    state.isSwitchingAccount = true;
    try {
        const res = await fetch(`/api/accounts/${targetIndex}/tier`, {
        ...
        ElMessage.error(t("accountTierUpdateFailed", { error: err.message || err }));
    } finally {
        state.isSwitchingAccount = false;
        updateContent();
    }
